...
WHERE ClientName = 'TechBooks GmbH' AND Status = 'New';

DELETE FROM OrderItem 
WHERE OrderID = (SELECT OrderID FROM Orders WHERE Status = 'Canceled' LIMIT 1);

DELETE FROM Orders 
WHERE Status = 'Canceled' LIMIT 1;
...
SELECT 
    o.OrderID AS 'Номер замовлення',
    o.OrderDate AS 'Дата',
    o.ClientName AS 'Клієнт',
    b.Title AS 'Книга',
    oi.Quantity AS 'Кількість',
    oi.UnitPrice AS 'Ціна за од.',
    (oi.Quantity * oi.UnitPrice) AS 'Загальна вартість'
FROM Orders o
